...
describe('OnboardingScreen Integration Tests', () => {
  ...
  it('should render first onboarding step', () => {
    ...
    expect(getByText('Take a photo of any math problem and get instant solutions')).toBeTruthy();
  });
  ...
  it('should complete onboarding and navigate to home', async () => {
    ...
    fireEvent.press(getByTestId('next-button'));
    await waitFor(() => expect(getByText('Practice with Quizzes')).toBeTruthy());

    // Complete
    fireEvent.press(getByTestId('next-button'));

    await waitFor(() => {
      expect(AnalyticsService.track).toHaveBeenCalledWith('onboarding_completed');
  ...
  it('should allow skipping onboarding', async () => {
    ...
    await waitFor(() => {
      expect(AnalyticsService.track).toHaveBeenCalledWith('onboarding_skipped', {
        step: 0,
      });
      expect(mockNavigate).toHaveBeenCalledWith('Home');
    });
